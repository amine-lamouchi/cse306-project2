  ...
  ~TriangleMesh() {}
    ...
    void center_scale() {
        ...
        for (int i = 0; i < vertices.size(); i++) {
            ...
            vertices[i] = vertices[i] / (maxBbox - minBbox).norm();
        }
    }
...
bool operator<(const Edge& e1, const Edge& e2) {
    int min1 = std::min(e1.a, e1.b);
    int min2 = std::min(e2.a, e2.b);
    int max1 = std::max(e1.a, e1.b);
    int max2 = std::max(e2.a, e2.b);
    return std::pair <int,int> (min1, max1) < std::pair <int,int> (min2, max2);
}
...
int main() {
    ...
    for (int i = 0; i < ordered_boundary_edges.size(); i++) {
        ...
        Vector circle_vertex;
        circle_vertex[0] = cos(theta)*0.5+0.5;
        circle_vertex[1] = sin(theta)*0.5+0.5;
        circle_vertex[2] = 0;
